...
if(!isset($_SESSION['login'])){//비 로그인사용자 방지
	header("Location:content.php?error=1");
}
...
if(isset($_POST[btndelete]) && $_POST[btndelete] !=""){
	...
	if($tmp == 0 || $tmp ==2){//새로고침으로인한 삭제버그 방지
		if($tmp+1 == 3){//DB자체삭제
			$query = "DELETE FROM messages WHERE outoID=$_POST[outoID]";//삭제 쿼리
		}else{//삭제 (숨김상태)
			$query = "UPDATE messages SET deletenum = $tmp+1 WHERE outoID=$_POST[outoID]";
		}
	}
...
<td class="key" width=10%><b>To</b></td>
...
<?php
$query = "SELECT * FROM messages WHERE mfrom='$_SESSION[login]' ORDER BY outoID DESC";
$result = mysqli_query($con,$query);
echo "<form method=post action=".$_SERVER[PHP_SELF].">";
$item=0;
while($row = mysqli_fetch_array($result)){
		if($item >= (($mpage*9)-9) && ($row[deletenum]==0 || $row[deletenum]==2)){//현재 페이지 게시물부터 출력 조건
			echo "<tr>";
			echo "<td class=value width=5% align=center><input type='radio' name='outoID' value='".$row[outoID]."' /></td>";
			echo "<td class=value width=10%>".$row[mto]."</td>";
...
echo "<br/><hr/>";
echo "</center>";
echo "<a class='send' href='msend.php'>　WRITE NEW MESSAGE　</a><hr/>";
